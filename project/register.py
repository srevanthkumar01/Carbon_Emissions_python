...
def Register():
    ...
    # Top Frame
    top_frame = Label(Reg, text='Registration', font=('Cosmic', 25, 'bold'), bg='WHITE', relief='groove', padx=500,
                      pady=30)
    ...
    def database(arg=None):
        ...
        if condition:
            ...
            if password != confirm:
                ...
                cursor.execute(
                    'CREATE TABLE IF NOT EXISTS Users (FullName TEXT NOT NULL, Email TEXT NOT NULL, Mobile TEXT NOT NULL, Username TEXT NOT NULL, Password TEXT NOT NULL)')

                # Inserting Data into Table
                cursor.execute('INSERT INTO Users (FullName, Email, Mobile, Username, Password) VALUES (?,?,?,?,?)',
    ...
    name_entry = tk.Entry(frame, font=('Arial', 12, 'normal'), bg='WHITE')
    name_entry.bind("<Return>", database)
    email_entry = tk.Entry(frame, font=('Arial', 12, 'normal'), bg='WHITE')
    email_entry.bind("<Return>", database)
    mobile_entry = tk.Entry(frame, font=('Arial', 12, 'normal'), bg='WHITE')
    mobile_entry.bind("<Return>", database)
    username_entry = tk.Entry(frame, font=('Arial', 12, 'normal'), bg='WHITE')
    username_entry.bind("<Return>", database)
    password_entry = tk.Entry(frame, font=('Arial', 12, 'normal'), show='*', bg='WHITE')
    password_entry.bind("<Return>", database)
    confirm_entry = tk.Entry(frame, font=('Arial', 12, 'normal'), show='*', bg='WHITE')
    confirm_entry.bind("<Return>", database)

    # Button that will call the submit function
    submit = tk.Button(frame, text='Register', command=database, width="10", bd='3', font=('Times', 12, 'bold'),
                       bg='BLACK', fg='white', relief='groove', justify='center', pady='5')
    ...
    Quit = tk.Button(Reg, text="Quit", width="10", command=Reg.destroy, bd='3', font=('Times', 12, 'bold'), bg='WHITE',
